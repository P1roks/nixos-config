# TODO: refactor this service to be a single mpd service which allows hosting multiple instances
{ config, lib, pkgs, ... }:
...
let
  ...
  mpdConf = pkgs.writeText "mpd.conf" ''
    ...
    ${lib.optionalString (cfg.dbFile != null) ''
      db_file             "${cfg.dbFile}"
    ''}
    state_file          "${cfg.dataDir}/stateCast"
...
in {
  ...
  config = mkIf cfg.enable {
    assertions = [
      (lib.hm.assertions.assertPlatform "services.mpd-cast" pkgs lib.platforms.linux)
    ];

    services.mpd-cast = mkMerge [
    ...
    systemd.user.services.mpd-cast = {
      Unit = mkMerge [
        {
          Description = "Music Player Daemon podcast instance";
